...
try:
    ...
    mycursor.execute(
        "select * from passenger_details where pnr_number = (%s)", (pnr,))
    pass_det = mycursor.fetchall()
    print("Passenger Details : ")
    passenger_details = PrettyTable(
        ['PNR', 'Name', 'Age', 'Gender', 'Seat_Number', 'Berth'])
    for t1 in pass_det:
    ...
    print(rows)
    mycursor.execute(
        "SELECT pnr_number,seat_number FROM passenger_details WHERE pnr_number = %s", (pnr,))
