...
try:
    ...
    if (mb == 1 and pd == 1 and gd == 1 and dd == 1 and ed == 1):
        cur.execute("INSERT INTO user_details(fname, lname, emailid, password, mobile_number, gender, dob,status) VALUES(%s, %s, %s, %s,%s,%s,%s,%s)",
                    (fname, lname, emailid, password, mobile_number, gender, dob, status))
        cur.execute("COMMIT")
        cur.execute("select user_id from user_details where fname = (%s) and lname = (%s) and emailid = (%s) and password = (%s) and mobile_number =(%s) and gender = (%s) and dob = (%s) and status = (%s)",
                    (fname, lname, emailid, password, mobile_number, gender, dob, status))
        ud = cur.fetchone()
        time.sleep(1)
